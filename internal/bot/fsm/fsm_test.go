...
func TestSetAndGet(t *testing.T) {
	...
	got := store.Get(42)
	if got.State != models.StateCertCreateP12 {
		t.Fatalf("expected %s, got %s", models.StateCertCreateP12, got.State)
	}
}
...
func TestTransitionSuccess(t *testing.T) {
	...
	// From idle to cert_create_p12
	ok := store.Transition(42, models.StateIdle, models.StateCertCreateP12)
	if !ok {
	...
	// From cert_create_p12 to cert_create_password
	ok = store.Transition(42, models.StateCertCreateP12, models.StateCertCreatePassword)
	if !ok {
		t.Fatal("transition should succeed")
	}
	if store.GetState(42) != models.StateCertCreatePassword {
		t.Fatal("state should be cert_create_password")
	}
}
...
func TestTransitionFail(t *testing.T) {
	...
	// Wrong 'from' state
	ok := store.Transition(42, models.StateCertCreatePassword, models.StateCertCreateProvision)
	if ok {
		t.Fatal("transition should fail for wrong 'from' state")
	}
	if store.GetState(42) != models.StateCertCreateP12 {
		t.Fatal("state should remain cert_create_p12")
	}
}
...
func TestCertCreateFlow(t *testing.T) {
	...
	steps := []struct {
		...
		{models.StateIdle, models.StateCertCreateP12},
