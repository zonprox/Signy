...
func getTestRedis(t *testing.T) *redis.Client {
	...
	// Clean test keys
	rdb.Del(ctx, StreamName) //nolint:errcheck
	return rdb
}
...
func TestAck(t *testing.T) {
	...
	consumer.EnsureGroup(ctx) //nolint:errcheck

	payload := EncodeJobPayload("job-ack-1", 100, "cs-1", "/test.ipa")
	producer.Enqueue(ctx, payload) //nolint:errcheck
...
func TestClaimStale(t *testing.T) {
	...
	consumer1.EnsureGroup(ctx)                                                     //nolint:errcheck
