...
func (s *Signer) Sign(ctx context.Context, jobID string, userID int64, certSetID, ipaPath, artifactBase string, options models.SigningOptions, ephemeralPassword string) (*SignResult, error) {
	signedPath := filepath.Join(artifactBase, "signed.ipa")
	logPath := filepath.Join(artifactBase, "sign.log")

	// Mock mode skips P12 decrypt entirely
	if s.mock {
	...
	if options.BundleName != "" {
		args = append(args, "-n", options.BundleName)
	}
	if options.BundleID != "" {
		args = append(args, "-b", options.BundleID)
	}
	if options.BundleVersion != "" {
		args = append(args, "-r", options.BundleVersion)
	}
	for _, dylib := range options.DylibPaths {
		args = append(args, "-l", dylib)
	}
