...
// DeriveSharedEphemeralKey derives a deterministic 32-byte key from a master key.
// This is used to create a shared ephemeral encryption key that both the Bot
// process and Worker process can independently derive from the same MASTER_KEY.
// This fixes the cross-process ephemeral password decryption failure that occurs
// when each process generates its own random processKey.
//
// When MASTER_KEY is set, use this instead of GenerateRandomKey() for processKey.
func DeriveSharedEphemeralKey(masterKey string) ([]byte, error) {
	return DeriveKey(masterKey, "signy-ephemeral-session-key-v1")
}
